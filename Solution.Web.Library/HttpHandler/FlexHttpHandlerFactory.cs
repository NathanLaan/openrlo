...
//using Solution.Web.Anetro.Library.Content;
...
namespace Solution.Web.Library.HttpHandler
{
  ...
  /// 
  /// </summary>
  public class FlexHttpHandlerFactory : IHttpHandlerFactory
  {
    ...
    {
      Match matchPage = FlexHttpHandlerFactory.PageRegex.Match(relativeUrl);
    ...
    private static List<FlexPage> flexPageList = new List<FlexPage>();

    public static void Add(FlexPage flexPage)
    {
      FlexHttpHandlerFactory.flexPageList.Add(flexPage);
    }

    private static List<FlexHttpHandlerPlugin> flexHttpHandlerPluginList = new List<FlexHttpHandlerPlugin>();

    public static void Add(FlexHttpHandlerPlugin flexHttpHandlerPlugin)
    {
      FlexHttpHandlerFactory.flexHttpHandlerPluginList.Add(flexHttpHandlerPlugin);
    }


    public FlexHttpHandlerFactory()
    {
    }

    public IHttpHandler GetHandler(HttpContext httpContext, string requestType, string url, string pathTranslated)
    {
      if (FlexHttpHandlerFactory.SiteOffline)
      {
        return new SiteOfflineHttpHandler(FlexHttpHandlerFactory.SiteOfflinePage);
      }
      ...
      if (!matchFound)
      {
        Match matchPage = FlexHttpHandlerFactory.PageRegex.Match(relativeUrlTrimmed);
        if (matchPage.Captures.Count > 0)
        {
          Capture capture = matchPage.Captures[0];
          if (capture.Value != null && capture.Value != string.Empty)
      ...
      if (FlexHttpHandlerFactory.Debug)
      {
        ...
        httpContext.Response.Write("FLEX-PATH: " + path + "<br/>\r\n");
        httpContext.Response.Write("FLEX-HOST: " + httpContext.Request.Url.Host + "<br/>\r\n");
        httpContext.Response.Write("FLEX-RAWU: " + httpContext.Request.RawUrl + "<br/>\r\n");
        httpContext.Response.Write("FLEX-RELU: " + relativeUrl + "<br/>\r\n");
        httpContext.Response.Write("FLEX-ACTU: " + url + "<br/>\r\n");
        httpContext.Response.Write("FLEX-QSTR: " + queryString + "<br/>\r\n");
        httpContext.Response.Write("</code>\r\n<br/>\r\n");
      }
